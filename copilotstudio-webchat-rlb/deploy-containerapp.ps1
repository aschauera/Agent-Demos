...
Write-Host "`n🏗️  Building Docker image locally: $imageNameWithTag" -ForegroundColor Cyan
Write-Host "ℹ️  Using --no-cache to ensure fresh build with latest files" -ForegroundColor Cyan

# Build the Docker image locally with --no-cache to avoid stale layers
docker build -t $imageNameWithTag -t $imageNameLatest --no-cache --platform linux/amd64 -f Dockerfile .
...
Write-Host "`n📤 Pushing timestamped image to ACR..." -ForegroundColor Cyan
docker push $imageNameWithTag

if ($LASTEXITCODE -ne 0) {
    Write-Error "Docker push failed (timestamped)"
    exit 1
}

Write-Host "📤 Pushing latest tag to ACR..." -ForegroundColor Cyan
docker push $imageNameLatest

if ($LASTEXITCODE -ne 0) {
    Write-Error "Docker push failed (latest)"
    exit 1
}
Write-Host "✅ Images pushed successfully" -ForegroundColor Green

# Use the timestamped image for deployment

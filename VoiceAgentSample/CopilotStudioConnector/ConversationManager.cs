...
namespace VoiceAgentSample
{
    ...
    public class ConversationManager
    {
        ...
        public async Task<RelayConversation> StartBotConversationAsync(string externalCID, IBotService botService)
        {
            ...
            var directLineBaseUri = new Uri(((BotService)botService).DirectLineBaseUri);
     
            using (var directLineClient = new DirectLineClient(directLineBaseUri,new DirectLineClientCredentials(token)))
            {
                var conversation = await directLineClient.Conversations.StartConversationAsync();
                string conversationId = conversation?.ConversationId;
                if (string.IsNullOrEmpty(conversationId))
                {
                    throw new TaskCanceledException($"Exception caught: ConversationID empty");
                }
